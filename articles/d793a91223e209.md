...
[divisions](https://api.flutter.dev/flutter/material/Slider/divisions.html)というパラメータで区切りの数から間隔を計算しtick markを描画します
...
* 標準のtick markを表示せず、trackの下に独自のラベルつきのtickを表示する
...
例えば[syncfusion_flutter_sliders](https://pub.dev/packages/syncfusion_flutter_sliders)のようなリッチなUIパッケージがありますが、商用利用の場合は金銭的なコストがかかってしまいます
...
今回は[RangeSlider](https://github.com/kentomiya89/slider_sample/blob/main/lib/custom_range_slider.dart)をカスタムした例になります
...
SliderThemeDataのtickMarkShapeに[SliderTickMarkShape.noTickMark](https://api.flutter.dev/flutter/material/SliderTickMarkShape/noTickMark.html)を設定してもRangeSliderのtick markは非表示にならないので注意です
...
もちろん、安定した3rdパーティパッケージがあればそれを使うのが最善ですが、要件を満たすパッケージがない場合でも、Flutterはデザインのカスタムに強いUIフレームワークな点を改めて感じました
